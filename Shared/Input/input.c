...
static void TimInputUpdateCallback(hal_tim_handle_t*);
...
hal_status_t Input_Add(input_t *pInput) {
  ...
  hal_status_t status;
  status = HAL_TIM_RegisterUpdateCallback(pInput->hTim, TimInputUpdateCallback);
  if (status != HAL_OK) {
    return status;
  }

  HAL_EXTI_Disable(pInput->hExti);
  status = HAL_EXTI_RegisterTriggerCallback(pInput->hExti,
...
void reportInputEvent(uint8_t ucEvent) {
  ...
  if (xQueueSendFromISR(sInputQueue, &ucEvent, &xHigherPriorityTaskWoken)
      == pdPASS) {
...
void ExtiInputTriggerCallback(hal_exti_handle_t *hexti,
  ...
  for (uint32_t i = 0; i < ulInputCount; i++) {
    if (pInputs[i]->hExti == hexti) {
      hal_tim_handle_t *hTim = pInputs[i]->hTim;
      if (hTim->global_state == HAL_TIM_STATE_ACTIVE) {
        // The state changed before the timer expired. Cancel the timer.
 ...
 */
void TimInputUpdateCallback(hal_tim_handle_t *htim) {
  for (uint32_t i = 0; i < ulInputCount; i++) {
    if (pInputs[i]->hTim == htim) {
      ...
      case GESTURE_NONE: {
        reportInputEvent(
            input->ucId | (ulPinState == HAL_GPIO_PIN_SET ?
