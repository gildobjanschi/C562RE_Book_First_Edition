...
system_status_t mx_gpio_default_init(void)
{
  ...
  /*
    ...
    PC13  ---------> PC13, B1
    */
  /* Configure PC8, PC13 GPIO pins in input mode */
  gpio_config.mode            = HAL_GPIO_MODE_INPUT;
  gpio_config.pull            = HAL_GPIO_PULL_NO;
  if (HAL_GPIO_Init(HAL_GPIOC, PC8_PIN | PC13_PIN, &gpio_config) != HAL_OK)
  {
    return SYSTEM_PERIPHERAL_ERROR;
  }
  ...
  /* Set the trigger as FALLING for the GPIOC */
  ...
  HAL_EXTI_Enable(&hEXTI8, HAL_EXTI_MODE_INTERRUPT);

  /* Set line 8 Interrupt priority */
  HAL_CORTEX_NVIC_SetPriority(EXTI8_IRQn, HAL_CORTEX_NVIC_PREEMP_PRIORITY_5, HAL_CORTEX_NVIC_SUB_PRIORITY_0);
...
{
  /* De-initialize the EXTI for GPIOC line8 */
