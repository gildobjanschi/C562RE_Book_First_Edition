/*******************************************************************************
 * file           : crc_task.c
 * brief          : The CRC task implementation.
 ******************************************************************************/
...
/*
 * @brief:  Exit the CRC task when there is an error
 *
 * @param error A description of the error that occurred
 */
static void exitCRCTask(char *error) {
  ErrorHandler(error);
 ...
 * @return HAL status
 */
static hal_status_t performCRC(CRC_PARAMETERS * params) {
  HAL_GPIO_WritePin(HAL_GPIOC, PC3_PIN, HAL_GPIO_PIN_SET);

  hal_crc_handle_t * pCRC = mx_crc_gethandle();
  uint32_t ulCrcValue = 0;

  hal_status_t hal_status = HAL_CRC_Calculate(pCRC, &DataBuffer,
      sizeof(uint32_t) * BUFFER_SIZE, &ulCrcValue);
...
/*
 * @brief:  The CRC task function
 *
 * @param pvParameters Task parameters
 */
static void vCRCTaskFunction(void *pvParameters) {
  ...
  while (1) {
    ...
    if ((uxBits & CRC_EV_GROUP_BIT) != 0) {
      if (performCRC(params) != HAL_OK) {
        exitCRCTask("performCRC failed\n");
        return;
      }
    }
  }
...
hal_status_t CRC_Init(CRC_PARAMETERS *params) {
  if (xTaskCreate(
      vCRCTaskFunction,   // Function that implements the task
      "CRC_Task",         // Text name for the task
