...
system_status_t mx_gpio_default_init(void)
{
  ...
  /*
    ...
    PC0   ---------> PC0
    */
  /* Configure PC0 GPIO pin in output mode */
  gpio_config.mode            = HAL_GPIO_MODE_OUTPUT;
  gpio_config.speed           = HAL_GPIO_SPEED_FREQ_LOW;
  gpio_config.pull            = HAL_GPIO_PULL_NO;
  gpio_config.output_type     = HAL_GPIO_OUTPUT_PUSHPULL;
  gpio_config.init_state      = PC0_INIT_STATE;
  if (HAL_GPIO_Init(PC0_PORT, PC0_PIN, &gpio_config) != HAL_OK)
  {
    return SYSTEM_PERIPHERAL_ERROR;
  }
  ...
  /*
    ...
    PC7   ---------> PC7
    */
  /* Configure PC7 GPIO pin in alternate mode for EVENTOUT */
  gpio_config.mode            = HAL_GPIO_MODE_ALTERNATE;
  gpio_config.speed           = HAL_GPIO_SPEED_FREQ_VERY_HIGH;
  gpio_config.pull            = HAL_GPIO_PULL_NO;
  gpio_config.output_type     = HAL_GPIO_OUTPUT_PUSHPULL;
  gpio_config.alternate       = HAL_GPIO_AF_15;
  if (HAL_GPIO_Init(PC7_PORT, PC7_PIN, &gpio_config) != HAL_OK)
  {
    return SYSTEM_PERIPHERAL_ERROR;
  }
...
system_status_t mx_gpio_default_deinit(void)
{
  ...
  HAL_GPIO_DeInit(HAL_GPIOC, PC0_PIN | PC7_PIN | PC13_PIN);
