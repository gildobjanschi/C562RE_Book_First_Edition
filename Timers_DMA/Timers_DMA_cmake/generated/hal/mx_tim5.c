...
hal_tim_handle_t *mx_tim5_init(void)
{
  ...
  if (HAL_TIM_OC_EnableComparePreload(&hTIM5, HAL_TIM_OC_COMPARE_UNIT_1) != HAL_OK)
  {
    return NULL;
  }
  ...
  if (HAL_TIM_SetCaptureCompareDMAReqSource(&hTIM5, HAL_TIM_CC_DMAREQ_CC) != HAL_OK)
  {
    return NULL;
  }
  ...
  hal_dma_direct_xfer_config_t xfer_cfg_tim5_cc1_dma;
  xfer_cfg_tim5_cc1_dma.request         = HAL_LPDMA1_REQUEST_TIM5_CC1;
  xfer_cfg_tim5_cc1_dma.direction       = HAL_DMA_DIRECTION_MEMORY_TO_PERIPH;
  xfer_cfg_tim5_cc1_dma.src_inc         = HAL_DMA_SRC_ADDR_INCREMENTED;
  xfer_cfg_tim5_cc1_dma.dest_inc        = HAL_DMA_DEST_ADDR_FIXED;
  xfer_cfg_tim5_cc1_dma.src_data_width  = HAL_DMA_SRC_DATA_WIDTH_WORD;
  xfer_cfg_tim5_cc1_dma.dest_data_width = HAL_DMA_DEST_DATA_WIDTH_WORD;
  xfer_cfg_tim5_cc1_dma.priority        = HAL_DMA_PRIORITY_LOW_WEIGHT_LOW;

  if (HAL_DMA_SetConfigPeriphDirectXfer(&hLPDMA1_CH0, &xfer_cfg_tim5_cc1_dma) != HAL_OK)
  {
    return NULL;
  }
  ...
  if (HAL_TIM_SetDMA(&hTIM5, HAL_TIM_DMA_ID_CC1, &hLPDMA1_CH0) != HAL_OK)
  {
    return NULL;
  }
